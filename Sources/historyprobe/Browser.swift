...
class Browser: Hashable {
    ...
    static func createSupportedBrowsers() -> [Browser] {
        ...
        let opera = Browser(type: .opera, appName: "Opera.app", longName: "Opera", processName: "opera", profileRoot: "/Library/Application Support/com.operasoftware.Opera/", storeName: "History.db", multipleProfiles: false, query: "SELECT hv.visit_time, hi.url, hv.title FROM history_items AS hi JOIN history_visits AS hv ON hi.id = hv.history_item ORDER BY hv.visit_time ASC", fingerprint: operaFingerprint)
